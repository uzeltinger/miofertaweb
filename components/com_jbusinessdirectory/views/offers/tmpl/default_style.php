...
<?php if($this->categoryId ){?>
 <div id="search-path">
	...
	<?php foreach($this->searchFilter["path"] as $path) {?>
		<li>
			<a href="<?php echo JBusinessUtil::getOfferCategoryLink($path[0], $path[2]) ?>"><?php echo $this->escape($path[1])?></a>
		</li>
	<?php } ?>
		<li>
			<?php  if(isset($this->category)) echo $this->escape($this->category->name) ?>
		</li>
	</ul>
</div>
<div class="clear"></div>
			...
			if(isset($this->searchFilter["categories"])){
				...
				<li>
					<?php if( isset($this->category) && $filterCriteria[0][0]->id == $this->category->id){ ?>
						<strong><?php echo $this->escape($filterCriteria[0][0]->name); ?>&nbsp;</strong><?php echo '('.$filterCriteria[1].')' ?>
					<?php }else{?>
					<a href="<?php echo JBusinessUtil::getOfferCategoryLink($filterCriteria[0][0]->id, $filterCriteria[0][0]->alias) ?>"><?php echo $this->escape($filterCriteria[0][0]->name); ?></a>
				...
				<?php foreach ($this->offers as $i=>$offer){?>
					<div class="offer-box span3" >
						...
						<div class="offer-subject">
							<a
								title="<?php echo $this->escape($offer->subject)?>"
								href="<?php echo JBusinessUtil::getOfferLink($offer->id, $offer->alias) ?>"><?php echo $this->escape($offer->subject)?>
								</a>
						</div>
