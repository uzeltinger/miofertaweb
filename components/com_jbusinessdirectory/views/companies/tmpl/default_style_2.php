		...
		<div class="row-fluid">		
			<div class="company-info-details span6">
				...
				<ul class="features-links">
					<li>
						<?php if($showData && (isset($this->package->features) && in_array(WEBSITE_ADDRESS,$this->package->features) || !$appSettings->enable_packages) && !empty($company->website)){
							...
							<strong><?php echo JText::_('LNG_WEBSITE') ?>: </strong><a <?php echo $followLink ?> itemprop="url" title="<?php echo $this->escape($this->company->name) ?> Website" target="_blank" onclick="increaseWebsiteClicks(<?php echo $company->id ?>)" href="<?php echo $this->escape($company->website) ?>"> <?php echo $this->escape($this->company->website) ?></a>
						<?php } else { ?>
				...
				<?php if(!empty($this->company->establishment_year)){?>
					<span>
						<strong><?php echo " ".JText::_('LNG_ESTABLISHMENT_YEAR') ?>:</strong><?php echo " ".$this->company->establishment_year;?>
					</span>
				<?php }?>
				...
				<?php if( $showData  && $appSettings->enable_attachments && (isset($this->package->features) && in_array(ATTACHMENTS, $this->package->features) || !$appSettings->enable_packages)) { ?>
					<?php if(!empty($this->company->attachments)) { ?>
						<strong><?php echo JText::_('LNG_ATTACHMENTS') ?>: </strong>
				...
				<?php if((isset($this->package->features) && in_array(CONTACT_FORM,$this->package->features) || !$appSettings->enable_packages) && !empty($company->email) && $appSettings->show_contact_form){ ?>
						<button type="button" class="ui-dir-button contact-btn" onclick="contactCompany(<?php echo $showData?"1":"0"?>)">
			...
			<div class="classification">
					<div class="categories">
						...
						<?php echo JText::_('LNG_TYPE')?>: <span><?php echo $this->escape($this->company->typeName) ?></span>
						<?php } ?>	
					</div>
				</div>
