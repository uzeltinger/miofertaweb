...
<div id="one-page-container" class="one-page-container" itemscope itemtype="http://schema.org/LocalBusiness">
	...
	<div class="row-fluid" style="margin-bottom: 0">
		<div id="company-info" class="company-info span9">
			...
			<div class="company-image" itemprop="logo" itemscope itemtype="http://schema.org/ImageObject">
				...
				<?php }else{ ?>
					<img title="<?php echo $this->escape($this->company->name) ?>" alt="<?php echo $this->escape($this->company->name) ?>" src="<?php echo JURI::root().PICTURES_PATH.'/no_image.jpg' ?>" itemprop="contentUrl">
				<?php } ?>
				...
				<?php if(false && $this->company->enableWorkingStatus && (!$appSettings->enable_packages || isset($this->package->features) && in_array(OPENING_HOURS,$this->package->features) && $this->company->enableWorkingStatus)) {
			...
			<div class="company-links">
				<ul class="features-links">
					<?php if($showData && (isset($this->package->features) && in_array(WEBSITE_ADDRESS,$this->package->features) || !$appSettings->enable_packages) && !empty($company->website)){
						...
						<li>
							<a <?php echo $followLink ?> target="_blank" itemprop="url" class="website" title="<?php echo $this->escape($this->company->name) ?> Website" onclick="increaseWebsiteClicks(<?php echo $company->id ?>)" href="<?php echo $this->escape($company->website) ?>"><?php echo JText::_('LNG_WEBSITE') ?></a>
						</li>
					<?php } else { ?>
					...
					if((isset($this->package->features) && in_array(CONTACT_FORM,$this->package->features) || !$appSettings->enable_packages) && !empty($company->email) && $appSettings->show_contact_form){ ?>
						<li>
		...
		<?php if($showData){?>
			<div class="company-map span3">
    			...
    			<?php if($appSettings->enable_reporting){?>
    				 <a class="report-listing" href="javascript:showReportAbuse()"><?php echo JText::_('LNG_REPORT_LISTING'); ?></a>
    			<?php } ?>
				...
				?>		
					<a href="javascript:showCompanyMap()" title="Show Map">
						<?php 
							...
							echo '<img class="static" src="https://maps.googleapis.com/maps/api/staticmap?center='.$this->company->latitude.','.$this->company->longitude.'&zoom=13&size=300x150&markers=color:blue|'.$this->company->latitude.','.$this->company->longitude.$key.'&sensor=false">';
						?>
					</a> 	
					<div class="clear"></div>
				<?php } ?>
	...
	<div class="company-menu">
		<nav>
			...
			if((isset($this->package->features) && in_array(COMPANY_SERVICES,$this->package->features) || !$appSettings->enable_packages)
				...
				?>
				<a id="events-link" href="javascript:showDetails('company-services');" class=""><?php echo JText::_("LNG_SERVICES")?></a>
			<?php } ?>
					...
					<ul>
						<?php
							foreach($this->company->categories as $i=>$category){
								?>
									<li> <a href="<?php echo JBusinessUtil::getCategoryLink($category[0], $category[2]) ?>"><?php echo $this->escape($category[1]) ?></a><?php echo $i<(count($this->company->categories)-1)? ',&nbsp;':'' ?></li>
								<?php 
							}
						?>
					</ul>
